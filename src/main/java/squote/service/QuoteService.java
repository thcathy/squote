...
public class QuoteService {
	...
	public QuoteService(CentralWebQueryService executeService,
		...
		super();
		this.queryService = executeService;
	...
	public List<Future<StockQuote>> getStockQuoteList(String codeList) {
		...
		for (String code : codes) futures.add(queryService.submit(new AastockStockQuoteParser(code)));				
    	return futures;
	}
	
	public Future<Map<HoldingStock, StockQuote>> getAllHoldingStocksWithMapping() {
		return queryService.submit(new Callable<Map<HoldingStock, StockQuote>>() {
